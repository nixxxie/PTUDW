...
<form action="xuli/xuli_SRC_1002E_add.php?id_REQUEST=<?php echo $id ?>" method="post">
  ...
  <select class="w3-select w3-gray" name="rate">
  ...
  <input type="text" name="note"value="Ghi Chú..." />
  </br>
  <input type="submit" name="submit"/>
  <form>


    <h3>Danh sách sinh viên được phân công</h3>
    <table class="w3-table">
      ...
      <?php while ($row1 = mysqli_fetch_assoc($sql_assigned_table)) { ?>
        <tr>
          ...
          <td><a href="layout_enterprise.php?xem=See_Student_profile&student_id=<?php echo $row1["id"] ?>&request_id=<?php echo $id ?>">Xem Chi Tiết</a></td> <?php // lam sau href="<?php echo $row1["id"] 
                                                                                                                                                                  ?>
